...
const SearchResultTitle = ({ resultCount, query }) => (
  <SearchHeader
    title={`‘${query}’`}
    label={`${resultCount} ${resultCount > 1 ? "resultater" : "resultat"} for`}
  />
);

const NoResultTitle = ({ query }) => (
  <SearchHeader label={`0 resultater for`} title={`‘${query}’`} />
);
        ...
        <NoResultContent>
          <Box maxW="600px" mx="auto">
            <Text fontSize="xl" textAlign="center">
              Vi fant dessverre ikke noe resultat for søket ditt. Du kan prøve
              et nytt søk.
            </Text>
            <SearchForm />
          </Box>
